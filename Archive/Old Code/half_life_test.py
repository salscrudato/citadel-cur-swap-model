...
half_life_values = range(110, 122, 1)
...
def run_backtest_for_half_life(hl_cnr, hl_lookback_tot, hl_lookback_mom, hl_mean_fx, hl_mean_msci, hl_mean_iv):
    ...
    for date in tqdm(dates, desc=f"Processing dates for half-life 170"):
...
hl_type = "cnr"
# Loop through the half-life values and save the results for each
# for hl in half_life_values:
results_list = run_backtest_for_half_life(174, 63, 63, 63, 63, 63)
results_df = pd.DataFrame(results_list)
output_path = f'Outputs2/output_{hl_type}_hl_170_2.csv'
results_df.to_csv(output_path, index=False)
print(f"Completed backtest for half-life")